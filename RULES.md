...
| Latch-like assignment pattern in `always_comb` | `NO_LATCH_IN_ALWAYS_COMB` | `SystemVerilog` | `block` | Flags `always_comb` blocks where a signal is assigned only inside a missing-`else` conditional without an earlier unconditional default assignment in the same block. | `src/pkg/rules/syntax/no_latch_in_always_comb.py` | `tests/rules/syntax/test_rules.py`, `tests/test_run_lint.py`, `tests/test_rule_registration.py` | First pass is intentionally heuristic: it only credits earlier unconditional same-block defaults, treats missing-`else` conditionals conservatively, and does not yet model full path coverage, `case` completeness, or richer control-flow equivalence. |
| `defparam` usage | `NO_DEFPARAM` | `Both` | `token` | Flags `defparam`, which is generally discouraged in favor of explicit parameter overrides at instantiation. | `src/pkg/rules/syntax/no_defparam.py` | `tests/rules/syntax/test_rules.py`, `tests/test_run_lint.py`, `tests/test_rule_registration.py` | Pure keyword check; it does not try to distinguish legacy codebases that intentionally permit `defparam`. |
