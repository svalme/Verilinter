...
| Implicit net | `NO_IMPLICIT_NET` | Flags unresolved identifiers currently modeled as implicit nets. | `src/pkg/rules/symbol/no_implicit_net.py` | `tests/rules/symbol/test_no_implicit_net.py`, `tests/test_run_lint.py` | Current implementation treats unresolved identifier uses as implicit nets broadly; it does not yet model `default_nettype` or finer language-specific implicit-net eligibility rules. |
| Multiple procedural drivers | `NO_MULTIPLE_DRIVERS` | Flags declared variables written from more than one procedural block. | `src/pkg/rules/symbol/no_multiple_drivers.py` | `tests/rules/symbol/test_no_multiple_drivers.py`, `tests/test_run_lint.py` | First pass only tracks distinct procedural writers; declaration initializers and other non-procedural write contexts are intentionally ignored for now. |
| Undeclared variable | `UNDECLARED_VARIABLE` | Flags used-but-undeclared symbols that are not already modeled as implicit nets. | `src/pkg/rules/symbol/undeclared_variable.py` | `tests/rules/symbol/test_undeclared_variable.py` | With the current semantic model, most parsed unresolved identifiers flow through `NO_IMPLICIT_NET`; this rule is more meaningful once the semantic layer distinguishes additional non-implicit undeclared cases. |
